...
use App\Models\MsmeApplication;
...
class MsmeController extends Controller
{
    ...
    {
        $applications = MsmeApplication::where('user_id', Auth::id())
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'bank_passbook' => 'required|string|max:255',
        ];
        ...
        try {
            ...
            $requestData=[
                ...
                'type' => 'DR',
                'amount' => 499,
                'description' => 'MSME Application',
                'transaction_id' => 'MSME' . rand(111111, 999999),
                'created_by' => $user->id,
            ...
            $data = $request->except([
                ...
                'bank_passbook', 'aplication_reciept'
            ]);
            ...
            $files = ['pancard_file', 'aadharcard_front', 'aadharcard_back', 'bank_passbook'];
            ...
            foreach ($files as $fileKey) {
                ...
                elseif ($request->hasFile($fileKey)) {
                    $result = $this->bunnyStorage->upload($request->file($fileKey), 'msme_documents');
                    if (!$result['success']) {
            ...
            $application = MsmeApplication::create($data);

            return response()->json([
                'status' => 1,
                'message' => 'MSME Application submitted successfully.',
                'data' => $application
            ]);
    ...
    {
        $application = MsmeApplication::with('user')->find($id);
    ...
    {
        $applications = MsmeApplication::with('user')->orderBy('created_at', 'desc')->get();
        return response()->json(['status' => 1, 'data' => $applications]);
    }

    public function adminDashboard()
    {
         $total = MsmeApplication::count();
         $pending = MsmeApplication::where('status', 0)->count();
         $approved = MsmeApplication::where('status', 1)->count();
         $rejected = MsmeApplication::where('status', 2)->count();
    ...
    public function dashboard()
     {
         $user_id = Auth::id();
         $total = MsmeApplication::where('user_id', $user_id)->count();
         $pending = MsmeApplication::where('user_id', $user_id)->where('status', 0)->count();
         $approved = MsmeApplication::where('user_id', $user_id)->where('status', 1)->count();
         $rejected = MsmeApplication::where('user_id', $user_id)->where('status', 2)->count();
    ...
    {
        $application = MsmeApplication::find($id);
        if (!$application) {
            return response()->json(['status' => 0, 'message' => 'Application not found'], 404);
        }
    ...
    public function uploadReceipt(Request $request, $id)
    {
        ...
        if ($request->hasFile('aplication_reciept')) {
            $result = $this->bunnyStorage->upload($request->file('aplication_reciept'), 'msme_receipts');
            if (!$result['success']) {
