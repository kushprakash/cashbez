<?php

// Test just the certificate and encryption logic without Laravel dependencies
echo "Testing the fixed RSA certificate...\n\n";

const PUBLIC_RSA_KEY = '-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----';

function encryptSessionKey($sessionKey)
{
    // First, parse the certificate to extract the public key
    $certificate = openssl_x509_read(PUBLIC_RSA_KEY);
    if (!$certificate) {
        throw new Exception("Invalid X.509 Certificate: " . openssl_error_string());
    }
    
    // Extract the public key from the certificate
    $publicKey = openssl_pkey_get_public($certificate);
    if (!$publicKey) {
        throw new Exception("Failed to extract public key from certificate: " . openssl_error_string());
    }
    
    // Encrypt the session key using RSA with PKCS1 padding
    if (!openssl_public_encrypt($sessionKey, $encrypted, $publicKey, OPENSSL_PKCS1_PADDING)) {
        throw new Exception("RSA Encryption failed: " . openssl_error_string());
    }
    
    return base64_encode($encrypted);
}

try {
    echo "1. Testing certificate parsing...\n";
    $certificate = openssl_x509_read(PUBLIC_RSA_KEY);
    if ($certificate) {
        echo "✓ Certificate parsing successful\n";
    } else {
        echo "✗ Certificate parsing failed: " . openssl_error_string() . "\n";
        exit(1);
    }
    
    echo "2. Testing public key extraction...\n";
    $publicKey = openssl_pkey_get_public($certificate);
    if ($publicKey) {
        echo "✓ Public key extraction successful\n";
        $details = openssl_pkey_get_details($publicKey);
        echo "   Key type: " . $details['type'] . "\n";
        echo "   Key bits: " . $details['bits'] . "\n";
    } else {
        echo "✗ Public key extraction failed: " . openssl_error_string() . "\n";
        exit(1);
    }
    
    echo "3. Testing session key encryption...\n";
    $sessionKey = random_bytes(16);
    echo "   Generated session key length: " . strlen($sessionKey) . " bytes\n";
    
    $encrypted = encryptSessionKey($sessionKey);
    echo "✓ RSA encryption successful!\n";
    echo "   Encrypted session key (base64) length: " . strlen($encrypted) . " characters\n";
    
    echo "\n🎉 SUCCESS: All tests passed!\n";
    echo "The AEPS onboard API RSA encryption should now work properly.\n";
    echo "The 'Invalid RSA Public Key' error has been resolved.\n";
    
} catch (Exception $e) {
    echo "✗ Test failed: " . $e->getMessage() . "\n";
    exit(1);
}
