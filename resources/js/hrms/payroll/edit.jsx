...
const PayrollEdit = () => {
  ...
  const fetchPayroll = async () => {
    setLoading(true);
    try {
      const res = await apiService.vGet(`/api/payroll/view/${id}`, true, true);
      if (res.data && res.data.status === 1) {
        setForm(res.data.data);
      } else {
        toast.error(res.data.message || 'Failed to fetch payroll');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await apiService.vPost(`/api/payroll/update/${id}`, form, true, true);
      if (res.data && res.data.status === 1) {
        toast.success('Payroll updated');
        navigate('/hrms/payroll/list');
      } else {
        toast.error(res.data.message || 'Failed to update payroll');
      }
    } catch (err) {
  ...
  return (
    <>
      <ToastContainer />
      <Pageheader mainheading="Payroll Management" parentfolder="Payroll" activepage="Edit Payroll" />
            ...
            <div className="col-md-12">
              <div className="card">
                <div className="card-header">
                  <h5 className="mb-0">Edit Payroll</h5>
                </div>
