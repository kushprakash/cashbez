...
const AddFollowupStatus = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let response;
            if (apiService.vPost) {
                response = await apiService.vPost('/api/crm/followup-statuses', submitData);
            } else if (apiService.post) {
                response = await apiService.post('/api/crm/followup-statuses', submitData);
            } else {
                const { token: authToken, user } = await import('../../core/auth/tokenManager').then(m => m.retrieveTokenAndUserData()) || {};
                const { axiosAuthorization } = await import('../../core/services/axiosConfig');
                const httpAuth = axiosAuthorization(authToken, user?.authorization);
                response = await httpAuth.post('/api/crm/followup-statuses', submitData);
            }
            ...
            if (error.response?.data?.errors) {
                ...
                toast.error('Error creating followup status. Please try again.');
            }
        } finally {
            setLoading(false);
        }
                                ...
                                <div className="form-group d-flex gap-2">
                                    ...
                                    >
                                        {loading ? (
                                            ...
                                            <>
                                                <i className="fa fa-save me-2"></i>Create Status
                                            </>
                                        )}
                                    </button>
