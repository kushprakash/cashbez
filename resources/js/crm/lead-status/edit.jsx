...
const EditLeadStatus = () => {
    ...
    const fetchLeadStatus = async () => {
        try {
            setPageLoading(true);
            const response = await apiService.vGet(`/api/crm/lead-status/${id}`);
            
            if (response.data) {
                const leadStatus = response.data;
                setFormData({
                    name: leadStatus.name || '',
                    color: leadStatus.color || '#6c757d',
                    followup_type_id: leadStatus.followup_type_id || '',
                });
            } else {
                toast.error('Lead status not found');
                navigate('/crm/lead-status/list');
            }
        } catch (error) {
            console.error('Error fetching lead status:', error);
            toast.error('Error fetching lead status data');
            navigate('/crm/lead-status/list');
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await apiService.vPut(`/api/crm/lead-status/${id}`, formData);
            ...
            toast.error(error.response?.data?.message || 'Error updating lead status. Please try again.');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="col-lg-6">
                    <div className="card">
                        <div className="card-header">
                            <h5 className="card-title mb-0">Edit Lead Status</h5>
                        </div>
