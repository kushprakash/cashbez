        ...
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1050 }}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
                <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                    <div className="modal-header text-white py-3 px-4" style={{ backgroundColor: '#6f42c1' }}>
                        <h5 className="modal-title fw-bold d-flex align-items-center gap-2">
                            <i className="bx bx-id-card fs-4"></i>
                            <span>RD Account Details & Member Profile</span>
                        </h5>
                    ...
                    <div className="modal-body p-4 bg-light">
                        ...
                        <div className="card border-0 shadow-sm rounded-3 p-3 mb-3 bg-white">
                            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <div>
                                    <span className="text-muted small d-block">Recurring Deposit Account No</span>
                                    <h4 className="fw-bold mb-0" style={{ color: '#6f42c1' }}>{account_number}</h4>
                                </div>
                        ...
                        <div className="row g-3">
                            <div className="col-md-6">
                                <div className="card border-0 shadow-sm rounded-3 p-3 h-100 bg-white">
                                    <h6 className="fw-bold border-bottom pb-2 mb-3" style={{ color: '#6f42c1' }}>
                            ...
                            <div className="col-md-6">
                                <div className="card border-0 shadow-sm rounded-3 p-3 h-100 bg-white">
                                    ...
                                    <div className="mb-2">
                                        <small className="text-muted d-block">Monthly Installment Amount</small>
                                        <strong className="text-dark fs-6">₹{parseFloat(opening_amount || 0).toLocaleString('en-IN')} / month</strong>
                                    </div>
                                    <div className="mb-2">
                                        <small className="text-muted d-block">Total Deposited</small>
...
const AccountStatementModal = ({ isOpen, account, onClose }) => {
    ...
    const handlePrintTxnReceipt = (t) => {
        ...
        const receiptObj = {
            ...
            service_type: account.service_type || 'RD',
            amount: t.amount,
            ...
            narration: t.narration || t.description || (isWithdrawal ? 'Withdrawal Transaction' : 'RD Deposit Transaction'),
                    ...
                    <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                        <div className="modal-header bg-info text-white py-3 px-4 no-print d-flex justify-content-between align-items-center">
                            <div className="d-flex align-items-center gap-2">
                                ...
                                <span className="fw-bold fs-5 text-white">RD Passbook Statement ({account.account_number})</span>
                            </div>
                        ...
                        <div className="modal-body p-4 bg-white" id="printable-passbook-statement">
                            ...
                            <div className="border-bottom pb-3 mb-3 text-center">
                                ...
                                <div className="mt-2 text-start p-2 bg-light rounded border d-flex justify-content-between text-dark small fw-bold flex-wrap gap-2">
                                    ...
                                    <span>Total Deposited: ₹{parseFloat(account.current_balance || 0).toLocaleString('en-IN')}</span>
                                </div>
                            </div>
                                        ...
                                        {loading ? (
                                            ...
                                            txns.map((t, idx) => (
                                                <tr key={t.id || idx}>
                                                    ...
                                                    <td className="text-muted">{t.narration || 'RD Deposit'}</td>
...
const calculateRdEstBalance = (acc) => {
    if (!acc) return { currentBal: 0, currentWithInterest: 0, accruedInterest: 0, maturityTotal: 0, rate: 7.5 };
    const currentBal = parseFloat(acc.current_balance || 0);
    const monthlyAmt = parseFloat(acc.opening_amount || 0);
    const rate = parseFloat(acc.interest_rate || 7.5);
    const months = parseInt(acc.duration_months || 12, 10);

    const matRes = calculateMaturity('RD', monthlyAmt, months, rate);
    return {
        currentBal,
        currentWithInterest: currentBal,
        accruedInterest: matRes.interest,
        maturityTotal: matRes.maturity,
        rate
    };
};
...
const RdAccountView = () => {
    ...
    return (
        <div className="container-fluid py-4">
            <div className="d-flex flex-wrap align-items-center justify-content-between mb-4 pb-2 border-bottom">
                <div>
                    <h3 className="fw-bold text-dark mb-1">
                        <i className="bx bx-refresh me-2" style={{ color: '#6f42c1' }}></i> Recurring Deposit (RD) Accounts
                    </h3>
                    <p className="text-muted mb-0">View RD accounts, check passbook statements, member details, and print deposit bonds</p>
                </div>
                <div className="d-flex gap-2">
                    ...
                    <button className="btn text-white fw-bold shadow-sm" style={{ backgroundColor: '#6f42c1', borderColor: '#6f42c1' }} onClick={() => setShowOpenModal(true)}>
            ...
            <div className="card border-0 shadow-sm rounded-3">
                <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="7" className="text-center py-4">Loading RD accounts...</td></tr>
                            ) : accounts.length > 0 ? (
                                accounts.map((acc) => {
                                    const est = calculateRdEstBalance(acc);
                                    return (
                                        <tr key={acc.id}>
                                            ...
                                            <td>
                                                <div>
                                                    <strong className="text-primary fs-6">
                                                        ₹{est.currentBal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                    </strong>
                                                    <span className="text-success d-block small fw-semibold" style={{ fontSize: '0.75rem' }}>
                                                        Est. Maturity: ₹{est.maturityTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="text-end">
                                                <div className="btn-group btn-group-sm">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setBondData({ ...acc, service_type: 'RD' });
                                ...
                                <tr><td colSpan="7" className="text-center py-4 text-muted">No Recurring Deposit accounts found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            {/* Open RD Modal (Exact same as Dashboard) */}
