...
const AccountDetailsModal = ({ isOpen, account, onClose }) => {
    ...
    const matRes = calculateMaturity('MIS', opening_amount, duration_months, interest_rate);
                ...
                <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                    <div className="modal-header bg-info text-white py-3 px-4">
                        <h5 className="modal-title fw-bold d-flex align-items-center gap-2">
                            ...
                            <span>MIS Account Details & Member Profile</span>
                        </h5>
                    ...
                    <div className="modal-body p-4 bg-light">
                        ...
                        <div className="card border-0 shadow-sm rounded-3 p-3 mb-3 bg-white">
                            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <div>
                                    <span className="text-muted small d-block">MIS Account No</span>
                                    <h4 className="fw-bold text-info mb-0">{account_number}</h4>
                                </div>
                            ...
                            <div className="col-md-6">
                                <div className="card border-0 shadow-sm rounded-3 p-3 h-100 bg-white">
                                    <h6 className="fw-bold text-success border-bottom pb-2 mb-3">
                                        <i className="bx bx-money me-2"></i>MIS Investment & Monthly Income
                                    </h6>
                                    <div className="mb-2">
                                        <small className="text-muted d-block">Principal Investment Amount</small>
                                    ...
                                    <div className="mb-2">
                                        <small className="text-muted d-block">Monthly Income Payout</small>
                                        <strong className="text-success fs-5">₹{matRes.monthlyPayout.toLocaleString('en-IN')} / month</strong>
                                    </div>
                                    <div className="mb-2">
...
const AccountStatementModal = ({ isOpen, account, onClose }) => {
    ...
    const handlePrintTxnReceipt = (t) => {
        ...
        const isWithdrawal = typeStr.includes('DEBIT') || typeStr.includes('WITHDRAW') || typeStr === 'DR' || narrationStr.includes('withdraw') || narrationStr.includes('debit') || narrationStr.includes('payout');
        ...
        const receiptObj = {
            ...
            service_type: account.service_type || 'MIS',
            amount: t.amount,
            ...
            narration: t.narration || t.description || (isWithdrawal ? 'Payout Transaction' : 'MIS Deposit Transaction'),
                    ...
                    <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                        <div className="modal-header bg-info text-white py-3 px-4 no-print d-flex justify-content-between align-items-center">
                            <div className="d-flex align-items-center gap-2">
                                ...
                                <span className="fw-bold fs-5 text-white">MIS Passbook Statement ({account.account_number})</span>
                            </div>
                        ...
                        <div className="modal-body p-4 bg-white" id="printable-passbook-statement">
                            ...
                            <div className="border-bottom pb-3 mb-3 text-center">
                                ...
                                <p className="text-muted small mb-0">MIS Monthly Income Passbook Transaction Statement</p>
                                <div className="mt-2 text-start p-2 bg-light rounded border d-flex justify-content-between text-dark small fw-bold flex-wrap gap-2">
                                    ...
                                    <span>Investment: ₹{parseFloat(account.opening_amount || 0).toLocaleString('en-IN')}</span>
                                </div>
                            </div>
                                        ...
                                        {loading ? (
                                            ...
                                            txns.map((t, idx) => (
                                                <tr key={t.id || idx}>
                                                    ...
                                                    <td className="text-muted">{t.narration || 'MIS Deposit / Payout'}</td>
...
const MisAccountView = () => {
    ...
    return (
        <div className="container-fluid py-4">
            <div className="d-flex flex-wrap align-items-center justify-content-between mb-4 pb-2 border-bottom">
                <div>
                    <h3 className="fw-bold text-dark mb-1">
                        <i className="bx bx-line-chart text-info me-2"></i> MIS (Monthly Income Scheme) Accounts
                    </h3>
                    <p className="text-muted mb-0">View MIS accounts, check passbook statements, member details, and print deposit bonds</p>
                </div>
                <div className="d-flex gap-2">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                accounts.map((acc) => {
                                    ...
                                    const matRes = calculateMaturity('MIS', principal, months, rate);
                                            ...
                                            <td>
                                                <div>
                                                    <strong className="text-success fs-6">
                                                        ₹{matRes.monthlyPayout.toLocaleString('en-IN', { minimumFractionDigits: 2 })} / month
                                                    </strong>
                                                    <span className="text-muted d-block small" style={{ fontSize: '0.75rem' }}>
                                                        Total Int: +₹{matRes.interest.toLocaleString('en-IN', { minimumFractionDigits: 2 })} ({rate}%)
                                                    </span>
                                                    <span className="text-primary d-block small fw-semibold" style={{ fontSize: '0.72rem' }}>
                                                        Est. Maturity Total: ₹{matRes.maturity.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="text-end">
                                                <div className="btn-group btn-group-sm">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setBondData({ ...acc, service_type: 'MIS' });
            ...
            {/* Open MIS Modal (Exact same as Dashboard) */}
