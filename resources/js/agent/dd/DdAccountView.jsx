...
const getPlanMinAmount = (p) => {
    if (!p) return 0;
    return parseFloat(p.minimum_opening_amount || p.minimum_installment || p.minimum_investment || p.minimum_daily_deposit || p.min_amount || 0);
};

const getPlanMaxAmount = (p) => {
    if (!p) return null;
    const max = p.maximum_opening_amount || p.maximum_installment || p.maximum_investment || p.maximum_daily_deposit || p.max_amount;
...
const DdAccountView = () => {
    ...
    const [mpinActionType, setMpinActionType] = useState(null); // 'OPEN_DD' | 'COLLECT_DD'
    ...
    const handleCollectClick = (e) => {
        ...
        if (amt <= 0) return toast.error('Please enter collection amount.');
        setMpinAmount(amt);
        setMpinActionType('COLLECT_DD');
        setShowMpinModal(true);
    };
    ...
    const handleMpinConfirm = async (mpinCode) => {
        ...
        try {
            ...
            } else if (mpinActionType === 'COLLECT_DD') {
    ...
    return (
        <div className="container-fluid py-4">
            <div className="d-flex flex-wrap align-items-center justify-content-between mb-4 pb-2 border-bottom">
                <div>
                    <h3 className="fw-bold text-dark mb-1">
                        <i className="bx bx-calendar-event text-warning me-2"></i> Daily Deposit (DD) Management
                    </h3>
                    <p className="text-muted mb-0">Open Daily Deposit accounts & post daily field collections via Utility Wallet</p>
                </div>
                <div className="d-flex gap-2">
                    ...
                    <button className="btn btn-warning text-dark fw-bold shadow-sm" onClick={() => setShowOpenModal(true)}>
            ...
            <div className="card border-0 shadow-sm rounded-3">
                <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="6" className="text-center py-4">Loading DD accounts...</td></tr>
                            ) : accounts.length > 0 ? (
                                accounts.map((acc) => (
                                    <tr key={acc.id}>
                                        ...
                                        <td className="text-end">
                                            <button className="btn btn-sm btn-warning text-dark fw-bold" onClick={() => openCollectModalForAccount(acc)}>
                                                <i className="bx bx-plus-circle me-1"></i> DD Collection
                                            </button>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr><td colSpan="6" className="text-center py-4 text-muted">No Daily Deposit accounts found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            <div className="modal-header bg-warning text-dark border-0 py-3 rounded-top-4">
                                <h5 className="modal-title fw-bold"><i className="bx bx-calendar-event me-2"></i> Open DD Account</h5>
                            ...
                            <form onSubmit={handleOpenClick}>
                                <div className="modal-body p-4">
                                            ...
                                            {plans.map((p) => {
                                                ...
                                                return (
                                                    <option key={p.id} value={p.id}>
                                                        {p.plan_name} ({p.plan_code || 'PLAN'}) {minAmt > 0 ? `- Min Daily: ₹${minAmt}` : ''}
                                                    </option>
                                                );
                                            })}
                                        </select>
                                    ...
                                    <div className="mb-3">
                                        <label className="form-label fw-semibold">Daily Deposit Amount (₹) <span className="text-danger">*</span></label>
                                        <input
                                            type="number"
                                            ...
                                            placeholder={openForm.min_amount ? `Min Amount ₹${openForm.min_amount}` : 'Enter Daily Amount'}
                                            required
                                        />
                                        {openForm.plan_id ? (
                                            <small className="text-primary d-block mt-1 font-semibold">
                                                <i className="bx bx-info-circle me-1"></i>
                                                Min Daily Amount: <strong>₹{openForm.min_amount}</strong>
                                ...
                                <div className="modal-footer bg-light border-0 py-3 px-4">
                                    ...
                                    <button type="submit" className="btn btn-warning text-dark px-4 fw-bold">Proceed to MPIN & Open</button>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            {showCollectModal && (
                <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            <div className="modal-header bg-warning text-dark border-0 py-3 rounded-top-4">
                                <h5 className="modal-title fw-bold"><i className="bx bx-plus-circle me-2"></i> Post DD Daily Collection</h5>
                                <button type="button" className="btn-close" onClick={() => setShowCollectModal(false)}></button>
                            </div>
                            <form onSubmit={handleCollectClick}>
                                <div className="modal-body p-4">
                                    ...
                                    <div className="mb-3">
                                        <label className="form-label fw-semibold">Collection Amount (₹) <span className="text-danger">*</span></label>
                                        <input
                                            type="number"
                                            className="form-control form-control-lg fw-bold"
                                ...
                                <div className="modal-footer bg-light border-0 py-3 px-4">
                                    ...
                                    <button type="submit" className="btn btn-warning text-dark px-4 fw-bold">Proceed to MPIN & Post Collection</button>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            <MpinModal
                ...
                title={mpinActionType === 'OPEN_DD' ? "Confirm & Open DD Account" : "Confirm DD Field Collection"}
                serviceTitle={mpinActionType === 'OPEN_DD' ? "Daily Deposit (DD) Opening" : "Daily Deposit Collection"}
                planName={mpinActionType === 'OPEN_DD' ? (openForm.plan_name || "Daily Deposit Scheme") : null}
                amount={mpinAmount}
                memberInfo={mpinActionType === 'OPEN_DD' ? members.find(m => m.id == openForm.member_id) : collectForm.member_name}
                accountInfo={mpinActionType === 'COLLECT_DD' ? collectForm.account_number : null}
