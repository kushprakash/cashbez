...
const ListModule = () => {
  ...
  const fetchModules = async () => {
    ...
    try {
      ...
      const response = await apiService.vGet('/api/modules');
      const { data } = response;
      if (data.status !== 1) {
  ...
  const handleDrop = async (e, dropIndex) => {
    ...
    try {
      const apiService = ApiService();
      const modulesWithOrder = items.map((module) => ({ id: module.id }));
      const response = await apiService.vPost('/api/modules/update-order', { modules: modulesWithOrder });
      ...
      toast.error(err.message || 'Failed to update module order');
      fetchModules();
    }
  };
  ...
  return (
    <>
      <Pageheader mainheading="Module Master" parentfolder="Modules" activepage="List" />
            ...
            <div className="col-md-12">
              <div className="card">
                <div className='card-header d-flex justify-content-between align-items-center rounded-top'>
                  <h3 className="mb-0 fw-bold">Modules</h3>
                        ...
                        <tbody>
                          {modules.map((module, index) => (
                            ...
                            >
                              ...
                              <td>{module.main_module?.name}</td>
                              <td>{module.name}</td>
                              <td>
                                {module.icon ? <i className={module.icon}></i> : 'No Icon'}
                              </td>
                              <td>
                                <span className={`badge ${module.status === 1 || module.status === '1' ? 'bg-success' : 'bg-secondary'}`}>
                                  {module.status === 1 || module.status === '1' ? 'Active' : 'Not Active'}
                                </span>
                              </td>
                              <td>
                                <Link
