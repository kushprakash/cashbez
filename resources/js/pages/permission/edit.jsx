...
const EditPermission = () => {
  ...
  const [initialSubModuleId, setInitialSubModuleId] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [permRes, mainModRes] = await Promise.all([
          apiService.vGet(`/api/module-permissions/${id}`),
          apiService.vGet('/api/main-modules'),
        ]);
        if (permRes.data.status !== 1) throw new Error(permRes.data.message || 'Failed to fetch permission');
        if (mainModRes.data.status !== 1) throw new Error(mainModRes.data.message || 'Failed to fetch main modules');
        
        setName(permRes.data.permission.name || '');
        setEndpoint(permRes.data.permission.endpoint || '');
        setRoute(permRes.data.permission.route || '');
        setStatus(permRes.data.permission.status !== undefined ? String(permRes.data.permission.status) : '1');
        setMenuShow(permRes.data.permission.menu_show !== undefined ? String(permRes.data.permission.menu_show) : '1');
        setModuleId(permRes.data.permission.module_id ? String(permRes.data.permission.module_id) : '');
        setInitialSubModuleId(permRes.data.permission.sub_module_id ? String(permRes.data.permission.sub_module_id) : '');
        setMainModules(mainModRes.data.modules || []);

        // If permission has a module_id, fetch the module details to get main_module_id
        if (permRes.data.permission.module_id) {
          const moduleRes = await apiService.vGet(`/api/modules/${permRes.data.permission.module_id}`);
          if (moduleRes.data && moduleRes.data.module && moduleRes.data.module.main_module_id) {
            ...
            setSelectedMainModule(String(mainModuleId));
    ...
    const fetchSubModules = async () => {
      ...
      try {
        ...
        if (response.data.status === 1) {
          ...
          setSubModuleId(prev => {
            // If already selected and exists, keep it
            if (prev && response.data.modules.some(sub => String(sub.id) === String(prev))) {
              return prev;
            }
            // If initialSubModuleId exists and valid, select it
            if (initialSubModuleId && response.data.modules.some(sub => String(sub.id) === String(initialSubModuleId))) {
              return String(initialSubModuleId);
            }
            return '';
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const apiService = ApiService();
      const response = await apiService.vPut(`/api/module-permissions/${id}`, formData, true, true, 'put');
      const { data } = response;
      if (data.status !== 1) {
  ...
  return (
    <>
      <Pageheader mainheading="Permission Master" parentfolder="Permissions" activepage="Edit" />
          ...
          <div className="col-md-12">
            ...
            <div className="card-header d-flex justify-content-between align-items-center rounded-top">
              <span className="d-flex align-items-center">
                ...
                <h5 className="mb-0 fw-semibold">Edit Permission</h5>
              </span>
            ...
            <div className="card-body p-4">
              <form onSubmit={handleSubmit} className='row'>
                ...
                <button type="submit" className="btn btn-primary w-100 text-white" disabled={loading}>
                  {loading ? 'Saving...' : <><i className="bi bi-plus-circle me-1"></i> Edit Permission</>}
                </button>
                {success && <div className="alert alert-success mt-3">Permission updated successfully!</div>}
