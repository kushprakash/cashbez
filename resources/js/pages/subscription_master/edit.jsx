...
const SubscriptionMasterEdit = () => {
    ...
    useEffect(() => {
        ...
        apiService.vGet(`/api/subscription-masters/${id}`).then(res => {
            if (res.data && res.data.data) {
                const data = res.data.data;
                // Explode comma-separated description string to array for add more system
                let descriptions = [];
                if (Array.isArray(data.descriptions)) {
                    descriptions = data.descriptions;
                } else if (typeof data.description === 'string') {
                    descriptions = data.description.split(',').map(d => d.trim());
                } else {
                    descriptions = [data.description || ''];
                }
                setForm({
                    main_module_id: data.main_module_id || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await apiService.vPut(`/api/subscription-masters/${id}`, submitData, true, true);
            if (res.data && res.data.status === 1) {
                navigate('/subscription-master/list');
            } else {
                if (res.data.error) {
    ...
    return (
        <>
            <Pageheader mainheading="Subscription Master" parentfolder="Subscription Plans" activepage="Edit" />
                            ...
                            <div className="card">
                                <div className="card-header d-flex justify-content-between align-items-center rounded-top">
                                    <span className="d-flex align-items-center">
                                        ...
                                        <h5 className="mb-0 fw-semibold">Edit Subscription Master</h5>
                                    </span>
                                ...
                                <div className="card-body p-4">
                                    <form onSubmit={handleSubmit} className="row">
                                        ...
                                        <div className="mb-3 col-md-3">
                                            ...
                                            <select className="form-control" name="sub_module_id" value={form.sub_module_id} onChange={handleChange} disabled={!form.module_id}>
...
export default SubscriptionMasterEdit;
