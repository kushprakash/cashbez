...
const AddUserRoleCommission = () => {
  ...
  // Fetch users and module tree with submodules and commissions
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [userRes, treeRes, mainModRes] = await Promise.all([
  ...
  // Fetch assigned commissions for user
  useEffect(() => {
    if (!userId) {
    ...
    const fetchUserCommissions = async () => {
      ...
      try {
        const apiService = ApiService();
        const assignedRes = await apiService.vPost(`/api/user-role-commissions/commissions`, { user_id: userId });
        if (assignedRes.data && assignedRes.data.status === 1) {
          setSelectedCommissions(
            (assignedRes.data.commissions || []).map(item => ({
  ...
  }, [userId, selectedTransactionType]);
  ...
  // Toggle all commissions for a submodule
  ...
  // Toggle all commissions for a module (all submodules)
  ...
  // Check if all commissions for a submodule are selected
  ...
  // Check if all commissions for a module are selected
  ...
  const handleAssignToAllUser = async () => {
    if (!window.confirm('Are you sure you want to assign commissions to ALL users based on their roles? This process might take a while.')) {
      return;
    }
    ...
    try {
      const apiService = ApiService();
      const response = await apiService.vPost('/api/user-role-commissions/assign-to-all');

      if (response.data.status === 1) {
        setSuccess(true);
        alert(response.data.message);
      } else {
        setError('Failed to assign commissions.');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiService = ApiService();
      await apiService.vGet('/api/user-role-commissions-delete-all?user_id=' + userId + '&TransactionType=' + selectedTransactionType);
      for (const sel of selectedCommissions) {
        const formData = new FormData();
        formData.append('user_id', userId);
        formData.append('role_id', users.find(u => u.id == userId)?.role || '');
        ...
        await apiService.vPost('/api/user-role-commissions', formData, true, true);
      }
      setSuccess(true);
    } catch (err) {
  ...
  return (
    <>
      <Pageheader mainheading="User Role Commission Master" parentfolder="User Role Commissions" activepage="Create" />
                ...
                <div className="card-body p-4">
                  <form onSubmit={handleSubmit} className="row">
                    ...
                    <button type="submit" className="btn btn-primary w-100 text-white mt-3" disabled={loading}>
                      {loading ? 'Saving...' : <><i className="bi bi-plus-circle me-1"></i> Save User Role Commissions</>}
                    </button>
                    {success && <div className="alert alert-success mt-3">User Role Commissions saved successfully!</div>}
...
export default AddUserRoleCommission;
