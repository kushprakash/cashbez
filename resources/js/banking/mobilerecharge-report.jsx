...
const MobileRechargeReport = () => {
    ...
    const fetchRecharges = async (page = 1, pageSize = 50) => {
        setLoading(true);
        try {
            const payload = {
                type: '1', // Default to Mobile if not specified
                ...filters,
                page: page,
                per_page: pageSize
            };
            ...
            if (response?.data?.status === 1) {
                ...
                if (rechargeData.length > 0 && operators.length <= 1) {
    ...
    const handlePageChange = (newPage) => {
        fetchRecharges(newPage, pagination.per_page);
    };

    const handlePerRowsChange = (newPerPage) => {
        fetchRecharges(1, newPerPage);
    };
    ...
    const viewInvoice = (recharge) => {
        ...
        const invoicePayload = {
            ...
            rechargeDetails: {
                ...
                operator: recharge.oprator,
                amount: recharge.amount,
                type: recharge.type === 1 || recharge.type === '1' ? 'Mobile Recharge' : 'DTH Recharge'
            },
            operatorDetails: {
                name: recharge.oprator,
                type: recharge.type === 1 || recharge.type === '1' ? 'Mobile' : 'DTH'
            },
            timestamp: recharge.created_at,
            user: recharge.user
        };
    ...
    const columns = [
        ...
        },
        {
            Header: 'Actions',
            accessor: 'actions',
            id: 'actions', // Explicit ID for exclusion
            Cell: ({ row }) => (
                <div className="d-flex gap-1">
                    ...
                    {row.original.status === 'pending' && (
                        <button
                            ...
                            onClick={() => checkStatus(row.original.txnid)}
                            title="Check Status"
                        >
    ...
    return (
        <>
            ...
            <Pageheader
                mainheading="Mobile Recharge Report"
                parentfolder="Banking"
                activepage="Mobile Recharge Report"
            />
                                    ...
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            {pagination && pagination.total > 0 && (
                                                <div className="row d-flex justify-content-between align-items-center mt-3 p-3 border-top">
                                                    ...
                                                    <div className="col-md-4 text-end">
                                                        <nav>
                                                            <ul className="pagination pagination-sm justify-content-end mb-0">
                                                                ...
                                                                <li className={`page-item ${!pagination.has_more_pages ? 'disabled' : ''}`}>
                                                                    <button
                                                                        className="page-link"
                                                                        onClick={() => handlePageChange(pagination.current_page + 1)}
