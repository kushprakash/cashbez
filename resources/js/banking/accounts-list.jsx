...
const ListAccounts = () => {
    ...
    const fetchAccountDetails = async (filterStart, filterEnd) => {
        try {
            ...
            if (!primaryAccount) {
                // If no primary account, maybe just return or handle gracefully
                setLoading(false);
                return;
            }
            setAccount(primaryAccount);
            let url = `/api/accounts/${primaryAccount.id}`;
            let params = {};
    ...
    const fetchAccounts = () => {
        setLoading(true);
        apiService.vGet(`/api/accounts`).then(res => {
                                        ...
                                        <div className="col-md-12">
                                            <div className="card">
                                                <div className="card-header d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div>
                                                        <button className="btn btn-sm btn-outline-primary" onClick={handleShowCreateModal}>Create Account</button>
                                                    </div>
                                                </div>
                                                            ...
                                                            <table className="table table-hover mb-0">
                                                                <thead className="table-light">
                                                                    <tr>
                                                                        ...
                                                                        <th>Status</th>
                                                                        <th className="d-none">Primary</th>
                                                                ...
                                                                <tbody>
                                                                    {accounts.map(acc => (
                                                                        <tr key={acc.id}>
                                                                            <td>{acc.name}</td>
                                                                            <td>{acc.number}</td>
                                                                            <td>₹{(parseFloat(acc.raw_balance) || 0).toFixed(2)}</td>
                                                                            <td>₹{(parseFloat(acc.raw_hold_amount) || 0).toFixed(2)}</td>
                                                                            <td>₹{(parseFloat(acc.raw_available_balance) || 0).toFixed(2)}</td>
                                                                            <td>
                                                                                <span className={`badge ${acc.status === 1 ? 'bg-success' : 'bg-secondary'}`}>
                                                                            ...
                                                                            <td className="d-none">
                                                                                {acc.is_primary ? (
                                                                                    ...
                                                                                    <button className="btn btn-sm btn-outline-primary" onClick={() => handleMakePrimary(acc)}>Make Primary</button>
                                                                                )}
                                                                            </td>
                                                                            <td className="text-end">
                                                                                <div className="btn-group" role="group">
                                                                                    <button className="btn btn-sm btn-info" onClick={() => handleViewAccount(acc)}>View</button>
                                                                                    <button className="btn btn-sm btn-secondary" onClick={() => handleChangeMpin(acc)}>Change MPIN</button>
                                                                                    <button className="btn btn-sm btn-danger d-none" onClick={() => handleDelete(acc.id)}>Delete</button>
                                                                                </div>
                                                                            </td>
                                                                        </tr>
                                                                    ))}
