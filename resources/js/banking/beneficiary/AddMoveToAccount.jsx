...
const AddMoveToAccount = ({ isEmbedded = false, onSuccess }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        ifsc: '',
        otp: '',
        type: 3  // 0 for customer, 1 for self
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.status === 1) {
                toast.success('Move to Account added successfully');
                if (isEmbedded && onSuccess) {
                    onSuccess();
                } else {
                    navigate('/banking/move-to');
                }
            } else {
                toast.error(response.data.message);
            ...
            toast.error(error.response?.data?.message || 'Failed to add Move to Account');
        } finally {
            setLoading(false);
        }
    };
    ...
    const content = (
        <div className={`p-3 card ${isEmbedded ? 'border-0 shadow-none' : ''}`}>
            <div className="card-header d-flex justify-content-between align-items-center rounded-top">
                <span className="d-flex align-items-center">
                    ...
                    <h5 className="mb-0 fw-semibold">Add Move To Account</h5>
                </span>
                {!isEmbedded && (
                    <Link to="/banking/move-to" className="btn btn-primary text-white d-flex align-items-center">
                        <i className="fa fa-list me-1"></i> Account List
                    </Link>
                )}
            </div>

            <div className={`P-3 card-body ${isEmbedded ? 'px-0' : ''}`}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="alert alert-info">
                        ...
                        <strong>Note:</strong> Once added, Account details cannot be edited or deleted. You can only view Account.
                    </div>
                    ...
                    <div className="d-flex justify-content-between">
                        {!isEmbedded && (
                            <button
                                ...
                                onClick={() => navigate('/banking/move-to')}
                            >
                        ...
                        {isEmbedded && (
                            <button
                                type="button"
                                className="btn btn-secondary"
                                onClick={() => onSuccess && onSuccess(false)} /* Pass false to indicate cancel/back */
                            >
                                Cancel
                            </button>
                        )}
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    <i className="fa fa-save me-1"></i> Add Move to Account
                                </>
                            )}
                        </button>
                    </div>
    ...
    return (
        <>
            <Pageheader
                mainheading="Add Move To Account"
                parentfolder="Banking"
                activepage="Add Move To Account"
            />
...
export default AddMoveToAccount;
